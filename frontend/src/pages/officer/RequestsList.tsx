...
export function OfficerRequestsList() {
    ...
    <OfficerLayout>
      <div className="p-6 space-y-5">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{requests.length} total</p>
        </div>

        <div className="flex gap-3 flex-wrap">
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="cf-input w-auto text-sm py-2">
            <option value="">All Statuses</option>
            {['PENDING_APPROVAL', 'IN_PROGRESS', 'ROUTED', 'APPROVED', 'ESCALATED'].map(s => (
